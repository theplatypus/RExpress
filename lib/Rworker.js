...
module.exports.Rworker = class {
	...
	constructor(id){
		...
		this.R.on('close', (code) => {
			...
			if(code !== 0) console.log('(#' + this.id + ') ' + '-- worker dead ; trying to resurrect ');
			if(this.thread[0]){
				this.thread[0].cb({err : "worker killed" , stack : this.thread[0].error}, null);
			}
		});
